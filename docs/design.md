...
`tests/headers/parity/` carries the fixtures of the cppast-based C++
reference implementation of this tool (`cppast_autopxd`: cppast/libclang
parse → pxd emission, validated on miniz/draco-style APIs). Each fixture is
generated by this Python implementation and validated with the real cython
compiler, locking in behavioral parity: the C typedef-struct idiom,
bitfields-as-plain-fields, enum `= value` members, class templates with
non-type parameters declared by name (`VectorD[ScalarT, dimension_t]`), free
function templates, const/non-const overload collapsing, and curated libc
imports.

## Known limitations

- Variadic templates (parameter packs) → skipped
- Alias templates (`template<class T> using X = ...`) → skipped as
  declarations (uses resolve through their canonical type)
- Member function templates → skipped
- Template template parameters → class/function skipped
- Conversion operators other than `operator bool()` → skipped
